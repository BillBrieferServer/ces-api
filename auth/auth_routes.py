"""
auth_routes.py
FastAPI routes for CES Idaho authentication.
...
def _get_csrf_functions():
    ...
    try:
        from main import _get_csrf_token, _validate_csrf_token, _require_csrf
...
def is_authorized_email(email: str) -> bool:
    """Check if email is authorized to register. Checks allowlist file."""
    ...
    if allowlist_path.exists():
        for line in allowlist_path.read_text().splitlines():
            if line.strip().lower() == email:
                return True
    # Check manual users JSON
    manual_path = DATA_DIR / "manual_users.json"
    if manual_path.exists():
        try:
            users = _json.loads(manual_path.read_text())
            if email in {k.lower() for k in users}:
                return True
        except Exception:
            pass
    return False
...
TRUSTED_DEVICE_COOKIE = "bb_trusted_device"
TRUSTED_DEVICE_DAYS = 30

# Templates (set by main app)
templates = None
...
async def register_submit(
    ...
    if not is_authorized_email(email):
        ...
        return templates.TemplateResponse("auth/register.html", {
            "request": request,
            "error": "Registration is by invitation only. Contact steve@quietimpact.ai for access.",
            "email": email
        })
...
async def register_password_submit(
    ...
    # Look up user info from manual_users.json or use email username
    user_info = _lookup_user(email)
...
def _lookup_user(email: str) -> dict:
    """Look up user info from manual_users.json or derive from email."""
    import json as _json2
    manual_path = DATA_DIR / "manual_users.json"
    if manual_path.exists():
        try:
            users = _json2.loads(manual_path.read_text())
