...
async def get_jurisdiction(jurisdiction_id: int, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(text("""
        ...
        FROM public.officials WHERE jurisdiction_id = :jid AND role_type = 'elected' AND ended_date IS NULL ORDER BY title, name
    """), {"jid": jurisdiction_id})
    ...
    result = await db.execute(text("""
        SELECT official_id, name, title, phone, email
        FROM public.officials WHERE jurisdiction_id = :jid AND role_type = 'staff' AND ended_date IS NULL ORDER BY title, name
    """), {"jid": jurisdiction_id})
