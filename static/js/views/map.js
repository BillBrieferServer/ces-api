...
import { ENTITY_LABELS, ENTITY_TYPE_ORDER } from "../shared.js";
...
async function showCountyPanel(countyId, countyName) {
  ...
  try {
    ...
    allTypes.forEach(type => {
      ...
      if (!items) return;
      html += `<div class="section-header" style="margin-top:12px">${ENTITY_LABELS[type] || type.replace(/_/g, " ")} (${items.length})</div>`;
      items.forEach(e => {
