...
export async function renderOfficials(el) {
  el.innerHTML = `
    ...
    <div id="off-list"><div class="empty">Search by name or select a title filter</div></div>
  `;
  ...
  async function load() {
    ...
    if (!name && !title) {
      listEl.innerHTML = `<div class="empty">Search by name or select a title filter</div>`;
      return;
    }
    ...
    listEl.innerHTML = data.map(o => `
      <div class="list-item" data-jid="${o.jurisdiction_name ? "" : ""}" data-name="${o.jurisdiction_name || ""}">
